...
import { ControlKind } from './discovery';
...
export class HomeApp {
  ...
  public identifyHandler = async (
    ...
    const discoveryData: IDiscoveryData = await this.getDiscoveryData(
      device,
  ...
  public reachableDevicesHandler = async (
    ...
    console.log(
      `REACHABLE_DEVICES request ${JSON.stringify(
    ...
    console.log(
      `REACHABLE_DEVICES response ${JSON.stringify(
  ...
  public executeHandler = async (
    ...
    await Promise.all(
      command.devices.map(async (device) => {
        ...
        const deviceCommand = makeSendCommand(
          customData.control_protocol,
          opcMessage
        );
        ...
        console.debug(
          `${customData.control_protocol} RequestData: `,
          deviceCommand
        );
        try {
  ...
  private getDiscoveryData = async (
    ...
    if (device.udpScanData !== undefined) {
      ...
      try {
        ...
        const httpResponseData = (await this.app
          .getDeviceManager()
          .send(deviceCommand)) as smarthome.DataFlow.HttpResponseData;
        const xmlResponse = httpResponseData.httpResponse.body as string;
        console.log('XML device description', xmlResponse);
        const deviceDescription = new DOMParser().parseFromString(
          xmlResponse,
          'text/xml'
        );

        // Parse UPnP type strings
        const deviceElement = deviceDescription.getElementsByTagName(
          'device'
        )[0];
        const deviceId =
          deviceElement
            .getElementsByTagName('UDN')[0]
            .textContent?.match(/uuid:([a-zA-Z0-9]+)/)?.[1] || '';
        const deviceModel =
          deviceElement.getElementsByTagName('modelName')[0].textContent || '';
        ...
        const channelList = Array.from(serviceElements).map((service) => {
          const channel = service
            .getElementsByTagName('serviceId')[0]
